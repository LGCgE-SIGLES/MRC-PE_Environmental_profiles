...
fichiers_tiff <- list.files("1 Donnees brutes/Metaux 1x1km/Dbrute 0-30 cm/Mediane", pattern = "\\.tif$", full.names = TRUE)
...
communes_RMQS_1km <- st_transform(communes, crs = st_crs(get(noms_rasters[1])))
...
# Extract raster cell values intersecting each municipality
  values_communes[[fichier]] <- extract(raster_obj, communes_RMQS_1km, small=TRUE, exact=TRUE)
  ...
  col_name <- fichier
  communes_RMQS_1km[[col_name]] <- moyennes
  i=i+1
  cat(i,",")
  }
...
st_write(communes_RMQS_1km, "3 Outputs/Metaux RMQS 1km/Mediane RMQS 1km avant gestion avant imputation.sqlite")